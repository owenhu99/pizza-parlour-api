...
def test_order():
    ...
    assert order.get_pizza().get_toppings() == ['olives']
...
def get_order():
    ...
    assert manager.get_order(order_num1).get_pizza().get_type() == 'pepperoni'
    assert manager.get_order(order_num2).get_pizza().get_type() == 'vegetarian'
